...
{
    static int lastValue = 55;
    float variation = (random(-30, 30) / 10.0); // -3.0 to 3.0
    lastValue += variation;
    if (lastValue > 70)
        lastValue = 70;
    if (lastValue < 40)
...
traffic["packetsPerSec"] = getRandomTraffic();
traffic["inBytes"] = random(50000, 150000);
traffic["outBytes"] = random(50000, 150000);

String jsonString;
broadcastJson(jsonString); // Send via WebSocket or HTTP
Serial.println("Network traffic Data sent: " + jsonString);
